...
export function InfoPage({eyebrow,title,intro,children}:{eyebrow:string;title:string;intro:string;children:React.ReactNode}) { return <><SiteHeader/><main className="info-page"><div className="info-hero"><span className="tag">{eyebrow}</span><h1 className="display">{title}</h1><p>{intro}</p></div><div className="info-content">{children}</div></main><SiteFooter/></> }

export function Section({title,children}:{title:string;children:React.ReactNode}){return <section className="info-section"><h2>{title}</h2>{children}</section>}
